...
def update_url_info():
    """Fetch URLs for the newly added cryptos, retrieve HTML content info, and update the database."""
    # Define the new cryptos you want to process
    new_cryptos = ["Tether", "Ethereum", "Solana", "Dogecoin"]

    # Fetch URLs from the database for the new cryptos where title is NULL
    cursor.execute("""
        SELECT id, url 
        FROM STORED_URLS 
        WHERE title IS NULL AND CRYPTO_NAME = ANY(%s);
    """, (new_cryptos,))
